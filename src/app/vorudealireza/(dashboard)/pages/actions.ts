...
const SANITIZE_OPTS: sanitizeHtml.IOptions = {
  allowedTags: ['p', 'br', 'strong', 'em', 'u', 's', 'a', 'ul', 'ol', 'li', 'blockquote', 'code', 'pre', 'h1', 'h2', 'h3', 'h4', 'img', 'figure', 'figcaption', 'hr', 'table', 'thead', 'tbody', 'tr', 'th', 'td', 'span'],
  allowedAttributes: { '*': ['href', 'src', 'alt', 'title', 'class', 'target', 'rel', 'width', 'height'] },
...
function parseBlocks(
  ...
  for (const b of arr.data) {
    ...
    if (b.type === 'richText' && typeof v.data.html === 'string') {
      v.data.html = sanitizeHtml(v.data.html, SANITIZE_OPTS);
    }
